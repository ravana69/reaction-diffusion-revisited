...
async function Draw(){
  ...
  if(!t){
    ...
    br = 1
    sp = cubels*.95
    base_shp = subDividedTetrahedron(cubels/2)
    space = Array(cl*rw*br).fill().map(v=>[2,1])
    for(j=1;j--;)for(i=99;i--;){
      ls = 5+j
      tx = (cl/2 + S(p=Math.PI*2/99*i)*ls)|0
      ty = (rw/2 + C(p)*ls)|0
      idx = (tx + ty*cl)
      idx = Math.min(space.length-1,Math.max(0,idx))
      space[idx|0] = [1,1]
    }
  ...
  oX=0, oY=0, oZ=Math.min(1.5,Math.max(.3,(.3+S(t))*5))
  Rl=-S(t/2)/3, Pt=-.2+C(t/3)*2, Yw = S(t/4)*3
  ...
  if(t>.5){
    ...
    //a
    if(Rn()<.25)for(flow=1;flow--;){
      ct = 0
      do{
        idx = space.length*Rn()|0
        ct++
      }while(ct<100&&space[idx][0]);
      space[idx] = [1,1]
    }
  ...
  threshold = .75
  for(m=1;m--;)JSON.parse(JSON.stringify(space)).map((v,i)=>{
    if(v[0]==1 && v[1]>threshold){
      ax1 = i%cl
      ay1 = (i/cl|0)%rw
      az1 = i/rw/cl|0
      for(j=6;j--;){
        ...
        az2 = az1
        switch(j){
          case 0: ax2 = ax1-1; break
          case 1: ay2 = ay1-1; break
          case 2: az2 = az1-1; break
          case 3: ax2 = ax1+1; break
          case 4: ay2 = ay1+1; break
          case 5: az2 = az1+1; break
        }
        if(ax2 < 0)   ax2 = cl-1
        if(ay2 < 0)   ay2 = rw-1
        if(az2 < 0)   az2 = br-1
        ...
        if(ay2 >= rw) ay2 = 0
        if(az2 >= br) az2 = 0
        tidx = ax2+ay2*cl+az2*rw*cl
        if((id = space[tidx][0]) && id == 2 && space[tidx][1]>threshold){
          space[tidx][0] = 1
          space[tidx][1] = 1
        }
      }
  ...
  space.map((v,i) => {
    if(v[0]){
      tx = ((i%cl)-cl/2+.5)*sp
      ty = (((i/cl|0)%rw)-rw/2+.5)*sp
      tz = ((i/cl/rw|0)-br/2+.5)*sp
      d = Math.hypot(tx/1.5,ty,tz)
      if(d<1)base_shp.map(q => {
        /*x.beginPath()
        q.map(n => {
          X = tx + n[0]
          Y = ty + n[1]
        ...
        col2 = `hsla(${(v[0]==1?180:0)+t*99},99%,50%,${.1*v[1]})`
        stroke('',col2,1,false)*/
        for(j=6;j--;){
          X = tx
          Y = ty
          ...
          switch(j){
            case 0: az = -sp1; break
            case 1: az = sp1; break
            case 2: yw = Math.PI/2, ax = -sp1; break
            case 3: yw = Math.PI/2, ax = sp1; break
            case 4: pt = Math.PI/2, ay = -sp1; break
            case 5: pt = Math.PI/2, ay = sp1; break
          }
          R(rl,pt,yw)
          X+=ax
          Y+=ay
          ...
          if(Z>0){
            s = Math.min(2e3, 70/Z)
            l = Q()
            x.globalAlpha = .25*v[1]
            x.fillStyle = `hsla(${(v[0]==1?0:180)+20*v[1]+t*100+i/5},99%,50%,.028)`
            x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
            s/=5
            x.fillStyle = `hsla(${(v[0]==1?0:180)+20*v[1]+t*100+i/5},99%,70%,.05)`
            x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
            s/=3
            x.fillStyle = '#fff'
            x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
      ...
      if(v[0]==1) v[1]-=killRate
      if(v[0]==2) v[1]+=killRate
